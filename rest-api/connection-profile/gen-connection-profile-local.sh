...
# This script uses the template connection-profile.json to generate a connection profile
# for the organisations in the Fabric network.

#REPODIR points to this repo.
REPODIR=~/Documents/apps/ledger.tts
...
cp connection-profile.yaml $REPODIR/tmp/connection-profile
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    sed -e "s|%REPODIR%|$CERTDIR|g" connection-profile.yaml > $REPODIR/tmp/connection-profile/connection-profile.yaml
else
    sed -i "s|%REPODIR%|$CERTDIR|g"  $REPODIR/tmp/connection-profile/connection-profile.yaml
fi 
